...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    ...
    >
      <div className="relative aspect-[3/4] bg-gradient-to-b from-blush to-cream overflow-hidden">
        <Image
          src="/model.jpg"
          alt={product.name}
          fill
        ...
        />
        {discount > 0 && (
          <span className="absolute top-3 left-3 bg-rose text-white text-xs font-medium px-2 py-1 rounded-full z-10">
            -{discount}%
          </span>
        )}
        {product.isNewArrival && (
          <span className="absolute top-3 right-3 bg-charcoal text-white text-xs font-medium px-2 py-1 rounded-full z-10">
            New
          </span>
        )}
      </div>
